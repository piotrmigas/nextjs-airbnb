...
type ProfileProps = {
  session: Session;
  bookings: { amount: number; id: string; items: BookingItemProps[]; timestamp: string }[];
};

const Profile = ({ session, bookings }: ProfileProps) => {
  if (!session) signIn();
        ...
        <section className='pt-6'>
          <div className='flex flex-col px-7'>
            ...
            {bookings.map(({ id, items, amount, timestamp }) => (
              <Booking key={id} items={items} amount={amount} timestamp={timestamp} />
            ))}
          </div>
        </section>
      </main>
